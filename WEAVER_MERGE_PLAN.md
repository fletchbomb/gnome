...
Yes — cleanup fits the overall plan, but in two layers:

- **Now (safe cleanup):** remove dead/duplicate selectors, remove stale comments, and isolate legacy v2 rules so they cannot interfere with v3.
- **Later (post-parity cleanup):** delete the old Weaver rendering path only after the v3 flow is validated across full house cycles.
